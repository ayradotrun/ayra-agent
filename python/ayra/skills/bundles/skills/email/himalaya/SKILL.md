...
This skill is separate from the AYRA Email gateway adapter. The gateway
adapter lets people email the agent and uses AYRA' built-in IMAP/SMTP
...
To reply non-interactively from AYRA, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from AYRA)** — pipe the message via stdin:
