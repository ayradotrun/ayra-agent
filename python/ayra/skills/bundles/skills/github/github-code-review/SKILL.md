...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _ayra_env="${AYRA_HOME:-$HOME/.ayra}/.env"; [ -f "$_ayra_env" ] && grep -q "^GITHUB_TOKEN=" "$_ayra_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_ayra_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from AYRA Agent\",
    \"comments\": [
...
```bash
source "${AYRA_HOME:-$HOME/.ayra}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by AYRA Agent. Code looks clean — good test coverage, no security concerns."
