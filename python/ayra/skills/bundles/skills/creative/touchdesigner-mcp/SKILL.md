...
metadata:
  ayra:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, ayra-video]
...
```
AYRA Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${AYRA_HOME:-$HOME/.ayra}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to AYRA config (if missing)
...
3. **Restart AYRA session** to pick up the new MCP server
