...
## 5. AYRA Configuration

### Config location

`$AYRA_HOME/config.yaml` (defaults to `~/.ayra/config.yaml` when `AYRA_HOME` is unset)
...
Restart the AYRA session for changes to take effect. The MCP connection is
