...
| Change | Upstream | AYRA |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `ayra` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of AYRA infra |
...
| File operations | Bash commands | AYRA file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains AYRA-specific adaptations.
