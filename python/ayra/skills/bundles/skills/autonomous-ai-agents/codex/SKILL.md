...
metadata:
  ayra:
    ...
    related_skills: [claude-code, ayra-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the AYRA terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For AYRA itself, `model.provider: openai-codex` uses AYRA-managed Codex
OAuth from `~/.ayra/auth.json` after `ayra auth add openai-codex`. For the
...
When invoking the Codex CLI from a AYRA gateway/service context (for example,
