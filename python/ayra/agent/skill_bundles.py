...
Storage
-------
Bundles live in ``~/.ayra/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``AYRA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``ayra bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under AYRA_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<AYRA_HOME>/skill-bundles``.
    """
    override = os.environ.get("HERMES_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_ayra_home() / "skill-bundles"
...
def build_bundle_invocation_message(
    ...
    from ayra.agent.skill_commands import _load_skill_payload, _build_skill_message
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `ayra bundles` CLI subcommand.
# ---------------------------------------------------------------------------
