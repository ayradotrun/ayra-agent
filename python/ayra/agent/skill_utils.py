...
from ayra.ayra_constants import get_ayra_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        try:
            from ayra.ayra_constants import is_container

            result = is_container()
        except Exception:
            result = False
    elif env == "s6":
        # The AYRA Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``ayra_cli.config`` on the
    skill prompt/build path. A tiny local cache gives the same repeated-read
    win without pulling the heavier CLI config stack into startup.
    """
    config_path = get_ayra_config_path()
...
# which becomes the dominant cost of ``ayra`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.ayra/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``ayra`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against AYRA_HOME, not cwd
        if not p.is_absolute():
            p = (ayra_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.ayra/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    ayra = metadata.get("ayra") or {}
    if not isinstance(ayra, dict):
        ayra = {}
    return {
        "fallback_for_toolsets": ayra.get("fallback_for_toolsets", []),
        "requires_toolsets": ayra.get("requires_toolsets", []),
        "fallback_for_tools": ayra.get("fallback_for_tools", []),
        "requires_tools": ayra.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes AYRA metadata, VCS, virtualenv/dependency, cache, and skill
