...
A *suggestion* is a ready-to-run cron job spec that AYRA surfaces to the
...
Storage mirrors ``cron/jobs.py``: ``~/.ayra/cron/suggestions.json``, atomic
...
CRON_DIR = get_ayra_home().resolve() / "cron"
...
def _save_raw(suggestions: List[Dict[str, Any]]) -> None:
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            json.dump(
                {"suggestions": suggestions, "updated_at": _ayra_now().isoformat()},
                f,
                indent=2,
            )
            f.flush()
    ...
    *,
    ...
    with _suggestions_lock:
        ...
        record = {
            ...
            "created_at": _ayra_now().isoformat(),
        }
    ...
    with _suggestions_lock:
        ...
        for s in suggestions:
            if s.get("id") == suggestion_id:
                s["status"] = status
                s["resolved_at"] = _ayra_now().isoformat()
                changed = True
                break
        if changed:
            _save_raw(suggestions)
...
def accept_suggestion(ref: str, *, origin: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    ...
    from ayra.cron.jobs import create_job
