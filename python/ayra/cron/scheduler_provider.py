...
class CronScheduler(ABC):
    ...
    def fire_due(self, job_id: str, *, adapters: Any = None, loop: Any = None) -> bool:
        ...
        """
        from ayra.cron.jobs import claim_job_for_fire, get_job
        from ayra.cron.scheduler import run_one_job
...
def resolve_cron_scheduler() -> "CronScheduler":
    ...
    name = ""
    try:
        from ayra_cli.config import cfg_get, load_config
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(self, stop_event, *, adapters=None, loop=None, interval=60):
        import logging
        from ayra.cron.scheduler import tick as cron_tick
        from ayra.cron.jobs import record_ticker_heartbeat

        logger = logging.getLogger("cron.scheduler_provider")
        logger.info("In-process cron scheduler started (interval=%ds)", interval)
        # Heartbeat once before the first sleep so `ayra cron status` sees a
